...
export const TabsList = ({ children, className }: { children: ReactNode; className?: string }) => (
  <div className={className} style={{ display: 'flex', gap: '8px' }}>{children}</div>
);
...
export const TabsTrigger = ({ value, children, active, setActive }: { value: string; children: ReactNode; active?: string; setActive?: (v: string) => void }) => (
  <button
    style={{ padding: '8px 16px', background: active === value ? '#333' : '#eee', color: active === value ? '#fff' : '#000', border: 'none', borderRadius: '8px' }}
