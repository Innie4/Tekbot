...
const ConversationVolumeChart = dynamic(() => import('./charts/ConversationVolumeChart'), {
  ssr: false,
  loading: () => <div className="h-64 rounded-md bg-muted animate-pulse" />,
});
const ResponseTimeChart = dynamic(() => import('./charts/ResponseTimeChart'), {
  ssr: false,
  loading: () => <div className="h-64 rounded-md bg-muted animate-pulse" />,
});
...
export default function AnalyticsDashboard() {
  ...
  const loadAnalyticsData = async () => {
    try {
      const response = await api.get<{ success: boolean; message: string; data: AnalyticsData }>(
        '/analytics/dashboard'
      );
  ...
  const conversationData =
    analyticsData?.conversationVolume?.map((d) => ({ name: d.date, volume: d.conversations })) ??
    [];
  const responseTimeData =
    analyticsData?.responseTimeData?.map((d) => ({ name: `${d.hour}:00`, time: d.responseTime })) ??
    [];
        ...
        <GlassCard className="p-6">
          ...
          <div className="h-64 border border-border/10 rounded-md bg-background">
            {conversationData.length ? (
              <ConversationVolumeChart data={conversationData} />
            ) : (
      ...
      <GlassCard className="p-6">
        ...
        <div className="space-y-4">
          {[
            ...
            <div
              key={index}
              className="flex items-center justify-between pb-3 border-b border-border/10"
            >
