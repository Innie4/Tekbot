      ...
      <GlassCard className="w-full max-w-2xl">
        ...
        <div className="grid grid-cols-4 gap-2 mb-8">
          {[1, 2, 3, 4].map((n) => (
            <div
              key={n}
              className={`h-2 rounded-full ${step >= n ? 'bg-electric-cyan' : 'bg-border'}`}
            />
          ))}
        </div>
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <button
                ...
                className={`flex flex-col items-center rounded-lg border p-4 transition ${dataSource === 'file' ? 'border-electric-cyan bg-electric-cyan/10' : 'border-border'}`}
              >
              ...
              <button
                ...
                className={`flex flex-col items-center rounded-lg border p-4 transition ${dataSource === 'url' ? 'border-electric-cyan bg-electric-cyan/10' : 'border-border'}`}
              >
              ...
              <button
                type="button"
                onClick={() => setDataSource('text')}
                className={`flex flex-col items-center rounded-lg border p-4 transition ${dataSource === 'text' ? 'border-electric-cyan bg-electric-cyan/10' : 'border-border'}`}
              >
            ...
            {dataSource === 'file' && (
              <div className="mt-4">
                <label htmlFor="onboarding-file" className="block text-sm mb-1">
                  Select a document
                </label>
                <input
                  id="onboarding-file"
            ...
            {dataSource === 'url' && (
              <div className="mt-4">
                <label htmlFor="onboarding-url" className="block text-sm mb-1">
                  Website URL
                </label>
                <GlassInput
                  id="onboarding-url"
            ...
            {dataSource === 'text' && (
              <div className="mt-4">
                <label htmlFor="onboarding-text" className="block text-sm mb-1">
                  Paste content
                </label>
                <textarea
                  id="onboarding-text"
        ...
        {step === 2 && (
          ...
          >
            ...
            <GlassInput
              type="text"
              placeholder="e.g., TekAssist Helper"
              value={botName}
        ...
        {step === 3 && (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {['Answer support questions', 'Generate leads', 'Drive sales', 'Educate users'].map(
                (goal) => (
                  <label
                    key={goal}
                    className={`flex items-center gap-3 rounded-lg border p-3 cursor-pointer ${botGoal === goal ? 'border-electric-cyan bg-electric-cyan/10' : 'border-border'}`}
                  >
                    <input
                      type="radio"
                      name="bot-goal"
                      className="accent-electric-cyan"
                      checked={botGoal === goal}
                      onChange={() => setBotGoal(goal)}
                    />
            ...
            {!done ? (
              <div className="flex flex-col items-center justify-center py-10">
                ...
                <p className="mt-4 text-sm text-muted-foreground">
                  Training your chatbot… {progress}%
                </p>
