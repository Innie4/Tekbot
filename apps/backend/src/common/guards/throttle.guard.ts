...
export class CustomThrottleGuard extends ThrottlerGuard {
  private static requestCounts: Record<
    string,
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const key =
      request.ip ||
      (request.user?.id ? `user-${request.user.id}` : 'anonymous');
    ...
    if (
      !CustomThrottleGuard.requestCounts[key] ||
      CustomThrottleGuard.requestCounts[key].windowStart !== windowStart
    ) {
    ...
    const remaining = Math.max(
      0,
      limit - CustomThrottleGuard.requestCounts[key].count,
    );
    ...
    if (CustomThrottleGuard.requestCounts[key].count > limit) {
      const customMessage = this.getCustomThrottleMessage(
        request,
        throttleConfig,
      );
  ...
  private getCustomThrottleMessage(
    request: Request,
    throttleConfig: ThrottleConfig | undefined,
  ): string {
