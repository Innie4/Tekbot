...
export default registerAs('reminder', (): ReminderConfig => ({
  ...
  inApp: {
    ...
    template: process.env.IN_APP_REMINDER_TEMPLATE || 'appointment-reminder-notification',
  },
  queue: {
    attempts: parseInt(process.env.REMINDER_QUEUE_ATTEMPTS || '3', 10),
    backoff: {
      type: (process.env.REMINDER_QUEUE_BACKOFF_TYPE as 'exponential' | 'fixed') || 'exponential',
      delay: parseInt(process.env.REMINDER_QUEUE_BACKOFF_DELAY || '2000', 10),
    },
    removeOnComplete: parseInt(process.env.REMINDER_QUEUE_REMOVE_COMPLETE || '100', 10),
    removeOnFail: parseInt(process.env.REMINDER_QUEUE_REMOVE_FAIL || '50', 10),
  },
  businessHours: {
