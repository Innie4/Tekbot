...
export const openaiConfig = registerAs('openai', () => ({
  ...
  models: {
    chat: {
      ...
      frequencyPenalty: parseFloat(process.env.OPENAI_CHAT_FREQUENCY_PENALTY) || 0,
      presencePenalty: parseFloat(process.env.OPENAI_CHAT_PRESENCE_PENALTY) || 0,
    },
    embedding: {
  ...
  // Rate limiting
  rateLimit: {
    requestsPerMinute: parseInt(process.env.OPENAI_REQUESTS_PER_MINUTE, 10) || 60,
    tokensPerMinute: parseInt(process.env.OPENAI_TOKENS_PER_MINUTE, 10) || 90000,
  },
  ...
  // Prompt templates
  prompts: {
    systemPrompt: process.env.OPENAI_SYSTEM_PROMPT || 'You are a helpful AI assistant for TekBot Platform.',
