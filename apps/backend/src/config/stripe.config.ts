...
export const stripeConfig = registerAs('stripe', () => ({
  ...
  supportedCurrencies: process.env.STRIPE_SUPPORTED_CURRENCIES ? 
    process.env.STRIPE_SUPPORTED_CURRENCIES.split(',') : ['usd', 'eur', 'gbp'],
  ...
  paymentMethods: {
    card: {
      ...
      confirmationMethod: process.env.STRIPE_CARD_CONFIRMATION_METHOD || 'automatic', // 'automatic' or 'manual'
    },
    applePay: {
  ...
  webhooks: {
    ...
    endpoints: {
      payments: process.env.STRIPE_WEBHOOK_PAYMENTS_ENDPOINT || '/api/v1/webhooks/stripe/payments',
      subscriptions: process.env.STRIPE_WEBHOOK_SUBSCRIPTIONS_ENDPOINT || '/api/v1/webhooks/stripe/subscriptions',
      customers: process.env.STRIPE_WEBHOOK_CUSTOMERS_ENDPOINT || '/api/v1/webhooks/stripe/customers',
    },
  },
  ...
  connect: {
    ...
    platformFeePercent: parseFloat(process.env.STRIPE_PLATFORM_FEE_PERCENT) || 2.9,
    applicationFeePercent: parseFloat(process.env.STRIPE_APPLICATION_FEE_PERCENT) || 0.5,
  },
