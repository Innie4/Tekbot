...
export const emailConfig = registerAs('email', () => ({
  ...
  templates: {
    ...
    unsubscribeUrl:
      process.env.EMAIL_UNSUBSCRIBE_URL || 'https://app.tekbot.com/unsubscribe',
    privacyPolicyUrl:
      process.env.EMAIL_PRIVACY_POLICY_URL || 'https://app.tekbot.com/privacy',
    termsOfServiceUrl:
      process.env.EMAIL_TERMS_URL || 'https://app.tekbot.com/terms',
    supportUrl:
      process.env.EMAIL_SUPPORT_URL || 'https://app.tekbot.com/support',
    ...
    types: {
      welcome: {
        subject:
          process.env.EMAIL_WELCOME_SUBJECT || 'Welcome to TekBot Platform!',
        template: 'welcome',
      },
      verification: {
        subject:
          process.env.EMAIL_VERIFICATION_SUBJECT || 'Verify your email address',
        template: 'email-verification',
        expiryHours:
          parseInt(process.env.EMAIL_VERIFICATION_EXPIRY_HOURS, 10) || 24,
      },
      passwordReset: {
        subject:
          process.env.EMAIL_PASSWORD_RESET_SUBJECT || 'Reset your password',
        template: 'password-reset',
        expiryHours:
          parseInt(process.env.EMAIL_PASSWORD_RESET_EXPIRY_HOURS, 10) || 1,
      },
      appointmentConfirmation: {
        subject:
          process.env.EMAIL_APPOINTMENT_CONFIRMATION_SUBJECT ||
          'Appointment Confirmed',
        template: 'appointment-confirmation',
      },
      appointmentReminder: {
        subject:
          process.env.EMAIL_APPOINTMENT_REMINDER_SUBJECT ||
          'Appointment Reminder',
        template: 'appointment-reminder',
      },
      appointmentCancellation: {
        subject:
          process.env.EMAIL_APPOINTMENT_CANCELLATION_SUBJECT ||
          'Appointment Cancelled',
        template: 'appointment-cancellation',
      },
      paymentConfirmation: {
        subject:
          process.env.EMAIL_PAYMENT_CONFIRMATION_SUBJECT ||
  ...
  queue: {
    ...
    removeOnComplete:
      parseInt(process.env.EMAIL_QUEUE_REMOVE_ON_COMPLETE, 10) || 100,
  ...
  bulk: {
    ...
    delayBetweenBatches:
      parseInt(process.env.EMAIL_BULK_DELAY_BETWEEN_BATCHES, 10) || 1000,
    maxRecipientsPerEmail:
      parseInt(process.env.EMAIL_BULK_MAX_RECIPIENTS, 10) || 50,
  },
  ...
  tracking: {
    ...
    unsubscribeTracking:
      process.env.EMAIL_UNSUBSCRIBE_TRACKING_ENABLED !== 'false',
  },
  ...
  spamPrevention: {
    ...
    maxEmailsPerHour:
      parseInt(process.env.EMAIL_MAX_EMAILS_PER_HOUR, 10) || 100,
  ...
  bounceHandling: {
    ...
    webhookUrl:
      process.env.EMAIL_BOUNCE_WEBHOOK_URL || '/api/v1/webhooks/email/bounce',
