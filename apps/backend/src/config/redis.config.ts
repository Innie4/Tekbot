...
export const redisConfig = registerAs('redis', () => ({
  ...
  retryDelayOnFailover:
    parseInt(process.env.REDIS_RETRY_DELAY_ON_FAILOVER, 10) || 100,
  ...
  pool: {
    ...
    acquireTimeoutMillis:
      parseInt(process.env.REDIS_POOL_ACQUIRE_TIMEOUT, 10) || 60000,
    createTimeoutMillis:
      parseInt(process.env.REDIS_POOL_CREATE_TIMEOUT, 10) || 30000,
    destroyTimeoutMillis:
      parseInt(process.env.REDIS_POOL_DESTROY_TIMEOUT, 10) || 5000,
    idleTimeoutMillis:
      parseInt(process.env.REDIS_POOL_IDLE_TIMEOUT, 10) || 30000,
    reapIntervalMillis:
      parseInt(process.env.REDIS_POOL_REAP_INTERVAL, 10) || 1000,
    createRetryIntervalMillis:
      parseInt(process.env.REDIS_POOL_CREATE_RETRY_INTERVAL, 10) || 200,
  },
}));
